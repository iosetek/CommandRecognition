...
def estimate_n_gaussians_from_mfcc_data(mfcc_data, n_gaussians, n_iterations=20):
    """
...
def __pick_random_gaussians(mfcc_data, n_gaussians):
    ...
    pi = 1 / n_gaussians
    sigma = np.array([[50, 0], [0, 5]], dtype=float)
...
def single_em_iteration(gaussians, mfcc_data):
    """
    Single Iteration Maximation step. If sigma_limit is not none
    then sigma matrice is divided.
    """
...
def __handle_sigma_with_limit(sigma, limit):
    current = math.sqrt(((sigma[0][0] ** 2) + (sigma[1][1] ** 2)))
    if current <= limit:
        return sigma
    ratio = current/limit
    sigma /= ratio
...
def __get_estimations(mfcc_data, gaussians):
    ...
    for i in range(len(gaussians)):
        for j in range(frames_count):
            for k in range(coefficients_count):
                if summed_estimations[j][k] != 0:
                    estimations[i][j][k] /= summed_estimations[j][k]
    return estimations
