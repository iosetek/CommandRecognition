...
class TestDiagonalMultivariateNormal(unittest.TestCase):
    ...
    def __test(self, mean, diag, pos, delta=1e-10, reserve=0):
        expected_pdf = mvn.pdf(pos, mean, diag)
        actual_pdf = DiagonalMultivariateNormal(mean, diag).pdf(pos, reserve) / (10**reserve)
    ...
    def test_setting_reserve(self):
        self.__test(mean=[8], diag=[2], pos=[5], reserve=1)
        self.__test(mean=[1, 1], diag=[1, 1], pos=[1, 1], reserve=2)
        self.__test(mean=[1, 1], diag=[1, 1], pos=[2, 2], reserve=-3)
        self.__test(mean=[1, 1], diag=[2, 1], pos=[1, 1], reserve=4)
        self.__test(mean=[1, 2, 3], diag=[1, 1, 1], pos=[3, 2, 1], reserve=5)
        self.__test(mean=[1, 2, 3], diag=[1, 3, 0.5], pos=[5, 5, 5], reserve=6)
        self.__test(mean=[1, 2, 3, 5, 8], diag=[2, 1, 20, 13, 99], pos=[5, 3, 3, 0.2, 18], reserve=7)
